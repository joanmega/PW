						...
						<?php
							if (isset($_REQUEST['bBuscar'])){
								...
								if ($_REQUEST['tLista'] == 0){
									$sentencia = 'SELECT * FROM usuario WHERE tipoUsuario = 2 ORDER BY apellido1, apellido2, nombre, dni;';		
								}else{
									$sentencia = 'SELECT * FROM usuario WHERE cod_usuario = "' .$_REQUEST['tLista']. '" and tipoUsuario = 2 ORDER BY apellido1, apellido2, nombre, dni;';		
								}
									$resultado = mysql_query($sentencia, $conexion);
									while ($fila = mysql_fetch_array($resultado)){
						?>

            ...
            <div id="cuerpo">
                ...
                <div id="contenido">
                    <fieldset>
						...
						<?php
							...
							if (isset($_REQUEST['bAceptar']) && $_REQUEST['tDNI']!='' && $_REQUEST['tApellido1']!='' && $_REQUEST['tApellido2']!='' && $_REQUEST['tNombre']!='' && $_REQUEST['DiaNac']!=0 && $_REQUEST['MesNac']!=0 && $_REQUEST['tUsuario']!='' && $_REQUEST['tContrasena']!='' && $_REQUEST['tContrasena2']!=''){
								...
								$sentencia = 'update Usuario, profesor 
								set pais="'.$pais.'", telefono="'.$telefono.'", movil="'.$movil.'", correoElectronico="'.$correoElectronico.'", fecNacimiento="'.$fecNacimiento.'", domicilio="'.$domicilio.'", usuario="'.$usuario.'", contrasena="'.md5($contrasena).'", extension='.$extension.', departamento="'.$departamento.'" 
								where DNI="'.$DNI.'" 
									and cod_profesor = cod_usuario;';
								...
								if (isset($_REQUEST['bBuscar']) || isset($_REQUEST['bAceptar'])){
						...
						?>
							<form id="modificarProfesor" method="post" name="modificarProfesor" action="modificarProfesor.php">
								...
								<tr>
                                    ...
                                    <td><input type="text" name="tExtension" onblur="validacion(tExtension);" size="30" maxlength="50" tabindex="7"<?php if(isset($_REQUEST['bAceptar'])) echo 'value="'.$_REQUEST['tExtension']. '"'; else echo 'value="'.$fila['extension']. '"';?>/></td>
                                </tr>
								<tr>
                                	<td><label>Departamento</label></td>
                                    <td><input type="text" name="tDepartamento" size="30" maxlength="50"<?php if(isset($_REQUEST['bAceptar'])) echo 'value="'.$_REQUEST['tDepartamento']. '"'; else echo 'value="'.$fila['departamento']. '"';?> tabindex="8"/></td>
                                </tr>
									<tr>
									...
									<tr>
										...
										<td><input name="tEmail" type="text" size="30" maxlength="50" tabindex="8" <?php if(isset($_REQUEST['bAceptar'])) echo 'value="'.$_REQUEST['tEmail']. '"'; else echo 'value="'.$fila['correoElectronico']. '"';?> onblur="validarEmail(tEmail);"/></td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td><input type="text" name="tUsuario" size="10" maxlength="10" <?php if(isset($_REQUEST['bAceptar'])) echo 'value="'.$_REQUEST['tUsuario']. '"'; else echo 'value="'.$fila['usuario']. '"'; ?> tabindex="13" required="required"/></td>
									</tr>      
									<tr>
									...
									<tr>
										<td><input type="hidden" name="tLista" value="<?php echo $_REQUEST['tLista']; ?>"/></td>
					...
					<?php
								...
								echo "No hay profesores dados de alta. <br>";
							}
						}else{
					?>
						<form id="buscarProfesor" method="post" name="buscarProfesor" action="modificarProfesor.php"> 
                            <table>
                                <tr>
                                    <td><label>Nombre</label></td>
                                    <td>
                                    <select name="tLista">
										<?php
											// Rellenamos el select del primer formulario para cargar los profesores.
											$sentencia = 'Select cod_usuario, dni, nombre, apellido1, apellido2 
											from usuario 
											Where tipoUsuario = 1';
											// Ejecutamos la consulta
											$resultado = mysql_query($sentencia, $conexion);
											// Rellenamos el select mientras haya registros.
